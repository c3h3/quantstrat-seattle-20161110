...
Total_Data = Total_Data[which(Total_Data$Labels != 0),]
...
library(e1071)
svmModel = svm(x = Training_X,y = Training_Y,cost = 10,gamma=1)
...
library(partykit)
plot(as.party(model$model$trees[[3]]))
...
library(spls)
model = splsda(x = as.matrix(Training_X),y = as.matrix(Training_Y),K = 5,eta = 0.9,kappa = 0.5)
model
table(Testing_Y,predict(model,Testing_X))
